...
describes what users get. Pick a format with `-o`, or let ostep choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ostep <command> -o table   # aligned columns for reading
ostep <command> -o jsonl   # one JSON object per line, for piping
ostep <command> -o json    # a single JSON array
ostep <command> -o csv     # spreadsheet friendly
ostep <command> -o tsv     # tab-separated
ostep <command> -o url     # just the URL column
ostep <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ostep <command> --fields id,title,url
```
...
```bash
ostep <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ostep <command>            # a table, because this is a terminal
ostep <command> | wc -l    # JSONL, because this is a pipe
```
